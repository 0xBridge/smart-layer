...
import {IMailbox, VulnerableContract} from "../../src/experimental/VulnerableContract.sol";
import {HyperlaneHelper} from "lib/pigeon/src/hyperlane/HyperlaneHelper.sol";
...
contract VulnerableContractTest is Test {
    ...
    function testInvalidSenderAttack() public {
        ...
        IMailbox(L1_HLMailbox).dispatch(
            L2_DOMAIN, bytes32(uint256(uint160(address(vulnerableContract)))), abi.encode(uint256(5))
        );
        ...
        // Check the state on the destination chain
        vm.selectFork(L2_FORK_ID);
        assertEq(VulnerableContract(vulnerableContract).value(), 5);
    }
}
