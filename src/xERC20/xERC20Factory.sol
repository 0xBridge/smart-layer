...
contract XERC20Factory is IXERC20Factory {
    ...
    /**
     * @notice The set of registered xEBTC tokens
     */
    EnumerableSet.AddressSet internal _xerc20RegistryArray;

    /**
     * @notice Deploys an xEBTC contract using CREATE3
        ...
        returns (address payable _lockbox)
    {
        ...
        if (xEBTC(_xerc20).owner() != msg.sender) revert IXERC20Factory_NotOwner();
    ...
    function _deployXERC20(
        ...
        bytes memory _creation = type(xEBTC).creationCode;
        ...
        for (uint256 _i; _i < _bridgesLength; ++_i) {
            xEBTC(_xerc20).setLimits(_bridges[_i], _minterLimits[_i], _burnerLimits[_i]);
        }

        xEBTC(_xerc20).transferOwnership(msg.sender);
    }
        ...
        returns (address payable _lockbox)
    {
        ...
        xEBTC(_xerc20).setLockbox(address(_lockbox));
